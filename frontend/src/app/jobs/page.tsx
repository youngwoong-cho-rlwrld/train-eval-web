...
import { api, type GpuQueueSnapshot, type Job, type JobProgress, type Progress } from "@/lib/api";
...
function ActiveProgressCell({ job }: { job: Job }) {
  ...
  if (!shouldFetch) {
    ...
    const label = /^PENDING$/i.test(job.state) && job.queue_position
      ...
      : state;
    if (/^PENDING$/i.test(job.state) && job.cluster !== "mlxp" && job.partition) {
...
function PendingQueueLabel({ job, fallbackLabel }: { job: Job; fallbackLabel: string }) {
  const queue = useQuery({
    queryKey: ["gpu-queue", job.cluster, job.partition],
    queryFn: () =>
      api<GpuQueueSnapshot>(
        `/api/clusters/${encodeURIComponent(job.cluster)}/gpu-queue?partition=${encodeURIComponent(job.partition)}`,
      ),
    staleTime: 15_000,
    refetchInterval: REFRESH_MS,
    retry: false,
...
function pendingQueuePositionLabel(state: string, position: number | null | undefined, total?: number) {
